...
def onSelect(self,event):
    ...
    with conn:
        ...
        # This returns a tuple and we can slice into 4 parts using data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.title() # This will ensure that the first character in each word is capatlized
    var_lname = var_lname.title()
    var_fullname = ('{} {}'.format(var_fname,var_lname)) # combine our normalized names into a fullname
    print('var_fullname: {}'.format(var_fullname)) # print to console
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            cursor = conn.cursor()
            # Check the databse for existence of the fullname, if so we will alert user and discard request
            cursor.execute('''SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = "{}"'''.format(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existence of fullname and we can add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel('Delete Confirmation','All information associated with, ({}) \nwill be permanently be deleted from database. \n\nProceed with the deletion process?'''.format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor = conn.cursor()
                    cursor.execute('''DELETE FROM tbl_phonebook WHERE col_fullname = "{}"'''.form(var_select))
...
def onClear(self):
    # clear the txt in these textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
...
def onUpdate(self):
    try:
        ...
        var_value = self.lstList1.get(var_select) # list slection's text value
    except:
        messagebox.showinfo('Missing selection','No name was seleceted from the list box. \nCancelling the Update request.')
        return
    # The user will only be allowed to update changes for phone and emails
    # For name changes, the user will need to delete the entirre record and start over.
    ...
    if(len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        messagebox.showerror('Missing information','Please select a name from the list. \nThen edit the phone of email information.')
    onClear(self)
